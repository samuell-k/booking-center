...
class HealthService {
  ...
  async checkMTNMoMo() {
    try {
      ...
      const response = await axios.get(
        `${process.env.MTN_MOMO_BASE_URL}/v1_0/apiuser`,
        {
          headers: {
  ...
  async checkRSwitch() {
    try {
      ...
      const response = await axios.get(
        `${process.env.RSWITCH_BASE_URL}/health`,
        {
          headers: {
  ...
  async runHealthCheck(checkName = null) {
    ...
    for (const [name, check] of this.checks) {
      promises.push(
        this.executeCheck(name, check).then(result => {
          results[name] = result;
        })
      );
    }
  ...
  determineOverallStatus(results, criticalChecks) {
    const criticalUnhealthy = criticalChecks.some(
