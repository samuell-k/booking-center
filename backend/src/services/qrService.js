    ...
    try {
      ...
      const stats = await query
        .select(
          ...
          db.raw('COUNT(CASE WHEN scan_result = \'valid\' THEN 1 END) as valid_scans'),
          db.raw('COUNT(CASE WHEN scan_result = \'invalid\' THEN 1 END) as invalid_scans'),
          db.raw('COUNT(CASE WHEN scan_result = \'already_used\' THEN 1 END) as duplicate_scans'),
          db.raw('COUNT(DISTINCT ticket_id) as unique_tickets'),
          db.raw('COUNT(DISTINCT scanner_id) as active_scanners')
        )
        .first();
