...
class PaymentService {
    ...
    async initiatePayment(paymentData) {
        const { user_id, event_id, ticket_ids, amount, payment_method, customer_phone, customer_email, customer_name, idempotency_key, reservation_token } = paymentData;
    ...
    async processMTNMoMoPayment(payment, phoneNumber) {
        try {
            ...
            const response = await axios.post(`${this.mtnConfig.baseURL}/collection/v1_0/requesttopay`, requestData, {
                headers: {
    ...
    async processAirtelMoneyPayment(payment, phoneNumber) {
        try {
            ...
            const response = await axios.post(`${this.airtelConfig.baseURL}/merchant/v1/payments/`, requestData, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
                    'Content-Type': 'application/json',
                    'X-Country': 'RW',
    ...
    async getAirtelAccessToken() {
        try {
            ...
            const response = await axios.post(`${this.airtelConfig.baseURL}/auth/oauth2/token`, {
    ...
    async processRSwitchPayment(payment) {
        try {
            ...
            const response = await axios.post(`${this.rswitchConfig.baseURL}/api/v1/payments/initiate`, requestData, {
                headers: {
                    'Content-Type': 'application/json',
    ...
    async checkMTNPaymentStatus(payment) {
        try {
            ...
            const accessToken = await this.getMTNAccessToken();
            const response = await axios.get(`${this.mtnConfig.baseURL}/collection/v1_0/requesttopay/${requestId}`, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
                    'X-Target-Environment': process.env.NODE_ENV === 'production' ? 'live' : 'sandbox',
                    'Ocp-Apim-Subscription-Key': this.mtnConfig.subscriptionKey
    ...
    async checkAirtelPaymentStatus(payment) {
        try {
            const accessToken = await this.getAirtelAccessToken();
            const response = await axios.get(`${this.airtelConfig.baseURL}/standard/v1/payments/${payment.external_reference}`, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
                    'X-Country': 'RW',
                    'X-Currency': 'RWF'
        ...
        return await withTransaction(async (trx) => {
            try {
                ...
                if (status === 'completed' && payment.reservation_token) {
                    await reservationService.confirmReservation(payment.reservation_token, payment.id);
                }
