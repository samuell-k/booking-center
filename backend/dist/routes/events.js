...
router.get('/', catchAsync(async (req, res) => {
    const { page = 1, limit = 20, sport, status = 'active', search, venue_id, team_id, start_date, end_date } = req.query;
    ...
    const eventsWithAvailability = await Promise.all(events.map(async (event) => {
        const ticketStats = await db('tickets')
            .select(db.raw('COUNT(*) as total_tickets'), db.raw('COUNT(CASE WHEN status = \'sold\' THEN 1 END) as sold_tickets'))
...
router.post('/', authenticate, authorize(['super_admin', 'venue_admin']), [
    ...
    const { title, description, sport, event_type, start_datetime, end_datetime, venue_id, home_team_id, away_team_id, ticket_price_regular, ticket_price_vip, ticket_price_student, max_tickets_per_user = 10, image_url } = req.body;
    // Validate datetime
